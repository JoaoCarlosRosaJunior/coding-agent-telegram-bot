...
for file in os.listdir(parent_directory + "/text/" + DOMAIN + "/"):

  # Open the file and read the text
  with open(parent_directory + "/text/" + DOMAIN + "/" + file, "r", encoding="UTF-8") as f:
    text = f.read()
...
df.to_csv(parent_directory + '/processed/scraped.csv')

# Load the cl100k_base tokenizer which is designed to work with the ada-002 model
tokenizer = tiktoken.get_encoding("cl100k_base")

df = pd.read_csv(parent_directory + '/processed/scraped.csv', index_col=0)
...
df.to_csv(parent_directory + '/processed/embeddings.csv')